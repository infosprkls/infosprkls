...
class Ajax extends Admin_Controller
{
    ...
    public function save()
    {
        ...
        if ($this->mdl_invoices->run_validation('validation_rules_save_invoice')) {
            ...
            if($invoice_category != "")
            {
                $query=$this->db->where('product_name',$invoice_category);
                $query=$this->db->get('ip_products');
                $invoice_category_exist=$query->result_array();

                if(count($invoice_category_exist) == 0)
                {
                    $this->db->insert('ip_products',['product_name'=>$invoice_category]);
                }
            }
            ...
            $db_array = [
                ...
                'instalment_price' => $this->input->post('instalment_price'),
                'instalment_description' => $this->input->post('instalment_description'),
                'invoice_category' => $this->input->post('invoice_category'),
            ...
            $invoice_balance = $this->mdl_invoice_amounts->calculate($invoice_id);

            $instalment_amount = 0;
            $instalment_due_dates = "";
            $num_of_instalments = $this->input->post('num_of_instalments');
            if($invoice_status != 4 && $num_of_instalments > 0 && $invoice_balance > 0)
            {
                $instalment_amount = $invoice_balance / $num_of_instalments;

                $invoice_date_due = $this->input->post('invoice_date_due');

                $instalment_due_dates = date('Y-m-d',strtotime($invoice_date_due));

                $last_due_date = $instalment_due_dates;

                for($i = 2; $i <= $num_of_instalments; $i++)
                {
                    $last_due_date = date('Y-m-d',strtotime('+1 month',strtotime($last_due_date)));
                    $instalment_due_dates .= ",".$last_due_date;
                }
            }
            ...
            $response = [
                ...
                'instalment_due_dates' => $instalment_due_dates
            ];
        } else {
